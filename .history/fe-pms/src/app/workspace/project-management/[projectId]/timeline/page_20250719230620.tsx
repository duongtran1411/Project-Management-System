...
const TimelinePage = () => {
  ...
  // Gom các sprint trùng ngày
  const sprintMap = new Map<
    string,
    { names: string[]; start: number; end: number }
  >();
  milestones.forEach((m) => {
    const key = `${new Date(m.startDate).getTime()}-${new Date(
      m.endDate
    ).getTime()}`;
    if (!sprintMap.has(key)) {
      sprintMap.set(key, {
        names: [m.name],
        start: new Date(m.startDate).getTime(),
        end: new Date(m.endDate).getTime(),
      });
    } else {
      sprintMap.get(key)!.names.push(m.name);
    }
  });
  // Group đầu là Sprints
  const sprintGroup = [{ id: "sprints", title: "Sprints" }];
  // Item dòng đầu: mỗi sprint (hoặc nhóm sprint trùng ngày) là 1 item, title là tên sprint nối nhau
  const sprintItems = Array.from(sprintMap.values()).map((s, idx) => ({
    id: `sprint-item-${idx}`,
    group: "sprints",
    title: s.names.join(", "),
    start_time: s.start,
  ...
  // Items các dòng epic: task thuộc epic, bar màu tím
  const epicItems = items.map((item: any) => ({
    ...item,
    style: { background: "#b39ddb", color: "#fff", borderRadius: 6 },
  }));
  const allGroups = [...sprintGroup, ...epicGroups];
  const allItems = [...sprintItems, ...epicItems];
