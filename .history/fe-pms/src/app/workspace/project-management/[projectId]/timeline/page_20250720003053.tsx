...
const TimelinePage = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const updatedItems = timelineItems.map(item => {
      ...
      switch (dragState.dragType) {
        case 'move':
          newStartDate = snapToDay(addDays(dragState.originalStartDate!, daysDelta));
          newEndDate = snapToDay(addDays(dragState.originalEndDate!, daysDelta));
          break;
        case 'resize-left':
          newStartDate = snapToDay(addDays(dragState.originalStartDate!, daysDelta));
  ...
  const handleMouseUp = async () => {
    ...
    const draggedItem = timelineItems.find(item => item.id === dragState.itemId);
    ...
    try {
      ...
      if (draggedItem.type === 'sprint') {
        ...
        const epicTasks = tasks.filter(task => task.epic === draggedItem.id);
        const daysDiff = differenceInDays(draggedItem.startDate, dragState.originalStartDate!);
              ...
              {milestones.map((milestone) => (
                ...
                >
                  ...
                  <div className="text-blue-600 text-xs mt-1">
                    ...
                    {format(new Date(milestone.endDate), "MMM dd")}
                  </div>
                </div>
              ))}
            </div>
