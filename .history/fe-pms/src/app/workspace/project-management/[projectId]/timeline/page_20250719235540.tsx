...
import { format, startOfWeek, getDay, addDays, differenceInDays, startOfMonth, endOfMonth, eachDayOfInterval } from "date-fns";
...
const TimelinePage = () => {
  ...
  useEffect(() => {
    ...
    const epicsWithDates: EpicWithDates[] = epics.map(epic => {
      ...
      const startDate = dates
        .filter(d => d.start)
        .reduce((min, d) => !min || d.start! < min ? d.start! : min, null as Date | null);
      
      const endDate = dates
        .filter(d => d.end)
        .reduce((max, d) => !max || d.end! > max ? d.end! : max, null as Date | null);
  ...
  const getItemPosition = (item: TimelineItem) => {
    const startOffset = Math.max(0, differenceInDays(item.startDate, viewRange.start));
    const endOffset = Math.min(totalDays, differenceInDays(item.endDate, viewRange.start) + 1);
