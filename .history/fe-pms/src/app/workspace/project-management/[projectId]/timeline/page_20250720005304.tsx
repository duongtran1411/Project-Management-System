...
const TimelinePage = () => {
  ...
  useEffect(() => {
    ...
    milestones.forEach((milestone) => {
      ...
      const endDate = milestone.dueDate ? new Date(milestone.dueDate) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days from now
          ...
          <div>
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {milestones.map((milestone) => {
                const startDate = milestone.startDate ? new Date(milestone.startDate) : new Date();
                const endDate = milestone.dueDate ? new Date(milestone.dueDate) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
