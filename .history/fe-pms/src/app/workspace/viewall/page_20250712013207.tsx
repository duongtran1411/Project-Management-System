...
const ProjectTable = () => {
  ...
  // Nếu searchTerm rỗng, lấy toàn bộ project của user, nếu có searchTerm thì gọi API search
  const {
    data: projectList,
    error,
    isLoading,
    ...
    debouncedSearch
      ? `${
        ...
        }${Endpoints.Statistics.SEARCH_BY_PROJECT(debouncedSearch)}`
      : userId
      ? `${
          process.env.NEXT_PUBLIC_API_URL
  ...
  return (
    <div className="p-6 bg-white rounded shadow">
      ...
      <div className="flex justify-center mt-4">
        <Pagination
          current={1}
          total={projectList?.data?.length || 0}
          pageSize={6}
        />
      </div>
    </div>
