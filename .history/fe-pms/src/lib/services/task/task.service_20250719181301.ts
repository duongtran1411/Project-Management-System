...
import { Task, TaskModel } from "@/models/task/task.model";
...
export const getTasksByMilestone = async (
  milestoneId: string
): Promise<Task[]> => {
  try {
    const response = await axiosService
      .getAxiosInstance()
      .get(Endpoints.Task.GET_BY_MILESTONE(milestoneId));
    // Chuyển đổi milestones từ string sang object {_id: string}
    const tasks: TaskModel[] = response.data?.data || [];
    return tasks.map((task) => ({
      ...task,
      milestones: task.milestones ? { _id: task.milestones } : undefined,
    })) as Task[];
  } catch (error: any) {
    const message =
      error?.response?.data?.message ||
      "Không thể lấy danh sách task theo Milestone!";
    showErrorToast(message);
    return [];
  }
};
