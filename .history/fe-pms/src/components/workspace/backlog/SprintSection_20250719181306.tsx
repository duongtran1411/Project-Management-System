...
import { getTasksByMilestone } from "@/lib/services/task/task.service";
...
const SprintSection: React.FC<Props> = ({
  ...
  const [milestoneTasksMap, setMilestoneTasksMap] = useState<
    Record<string, Task[]>
  >({});
  const [milestoneTasksErrorMap, setMilestoneTasksErrorMap] = useState<
    Record<string, boolean>
  >({});
    ...
    const fetchTasks = async () => {
      ...
      await Promise.all(
        milestoneData.map(async (milestone) => {
          try {
            const tasks = await getTasksByMilestone(milestone._id);
            tasksMap[milestone._id] = tasks || [];
            errorMap[milestone._id] = false;
          } catch {
            tasksMap[milestone._id] = [];
            errorMap[milestone._id] = true;
          }
        })
      );
      setMilestoneTasksMap(tasksMap);
      setMilestoneTasksErrorMap(errorMap);
    };
    fetchTasks();
  }, [milestoneData]);
  ...
  return (
    <div>
      {milestoneData?.map((milestone: Milestone) => {
        ...
        const taskInMileStone = milestoneTasksMap[milestone._id] || [];
        const milestoneTasksError = milestoneTasksErrorMap[milestone._id];
