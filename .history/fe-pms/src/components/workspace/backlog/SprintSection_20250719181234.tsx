  ...
  return (
    <div>
      {milestoneData?.map((milestone: Milestone) => {
        ...
        // Call API lấy task theo milestoneId
        const { data: milestoneTasks, error: milestoneTasksError } = useSWR(
          milestone._id ? ["tasks-by-milestone", milestone._id] : null,
          () => getTasksByMilestone(milestone._id)
        );
        const taskInMileStone = milestoneTasks || [];
