...
const menuItems = [
  ...
  { key: "Backlog", label: "Backlog", icon: <UnorderedListOutlined />, url:'backlog' },
  { key: "Board", label: "Board", icon: <TableOutlined /> ,url:'board' },
    ...
    <div className="w-full px-4 pt-3 bg-white shadow">
      ...
      <Menu
        ...
        items={menuItems.map((item) => ({
          ...
          label: (
            <span
              className="flex items-center gap-1 pr-1 text-sm font-semibold text-[#505258]"
              onClick={() =>
                router.push(
                  `/workspace/project-management/${item.url}
                    .toLowerCase()
                    .replace(/\s+/g, "-")}`
                )
              }>
